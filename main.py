...
assert LooseVersion(tf.__version__) >= LooseVersion('1.0'), 'Please use TensorFlow version 1.0 or newer.  You are ' \
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # upsample
    layer4_t = tf.layers.conv2d_transpose(layer7_out, num_classes, 4,
                                             strides=(2, 2),
                                             padding='same',
                                             kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer4_ew = tf.add(layer4_t, layer4_out)
    # upsample
    layer3_t = tf.layers.conv2d_transpose(layer4_ew, num_classes, 4,
                                             strides=(2, 2),
                                             padding='same',
                                             kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer3_out = tf.layers.conv2d(vgg_layer3_out, num_classes, 1,
                                   ...
                                   kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # skip connection (element-wise addition)
    layer3a_out = tf.add(layer3_t, layer3_out)
    # upsample
...
def run():
    ...
    with tf.Session() as sess:
        ...
        means = train_nn(sess, EPOCHS, BATCH_SIZE, get_batches_fn, train_op, cross_entropy_loss, input_image,
                         correct_label, keep_prob, learning_rate)
